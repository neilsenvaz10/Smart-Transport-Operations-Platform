...
export default function UsersScreen() {
 ...
 <div className="flex bg-slate-100/80 p-1.5 rounded-xl border border-slate-200/60 w-fit shadow-sm">
 <button
 onClick={() => setActiveTab('all')}
 className={`flex items-center gap-2 px-5 py-2 text-[13px] font-semibold rounded-lg transition-all duration-200 ${
 activeTab === 'all' 
...
function UserTable({ users, isLoading, onToggleStatus }: { users: UserData[], isLoading: boolean, onToggleStatus: (id: string) => void }) {
 ...
 <p className="text-slate-500 mt-1">There are no users in this category.</p>
 </div>
 );
 }
 ...
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">Name</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">Role</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">Status</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm">Last Login</th>
 <th className="py-3 px-4 font-medium text-slate-500 text-sm text-right">Actions</th>
 </tr>
 </thead>
...
function InviteUserForm({ onSuccess }: { onSuccess: () => void }) {
 ...
 <p className="text-sm text-slate-500 ">Invite a new team member via email.</p>
 </div>
 </div>
 ...
 type="text"
 {...register('name')}
 className="w-full text-sm rounded-lg py-2 px-3 border border-slate-300 bg-white text-slate-900 focus:ring-blue-500 focus:border-blue-500 outline-none"
 />
